...
const sourceItems=list=>Object.values(list?.items||{}).filter(x=>x?.product&&!EXCLUDED.has(String(x.product).trim().toUpperCase())).map(x=>({product:x.product,color:x.color||'SEM COR',qty:Number(x.total)||0})).filter(x=>x.qty>0);
