...
// Relatório consolidado da Costura.
// Duplicatas idênticas do mesmo envio são consideradas uma única vez.
(function(){
  ...
  const buildMatrix=rows=>{
    ...
    const map=new Map();for(const r of colorRows){const key=`${r.material}|${r.spec}|${r.unit}`;let g=map.get(key);if(!g){g={material:r.material,spec:r.spec,unit:r.unit,byColor:new Map()};map.set(key,g)}g.byColor.set(r.color,(g.byColor.get(r.color)||0)+r.qty)}
  ...
  function patch(){
    ...
    const sel=panel.querySelector('#report-seamstress'),selected=sel?sel.value:'TODAS',lots=uniqueLots(readLots().filter(l=>selected==='TODAS'||norm(l.costureira)===norm(selected)));if(!lots.length)return;
    ...
    const uniqueLists=[...new Set(lots.map(l=>String(l.listName||'').replace(/^\d+\s*(?:[-–—:]\s*)?/,'').trim()).filter(Boolean))];
    ...
    const metas=report.querySelectorAll('.report-meta');if(metas.length>=2){metas[0].innerHTML='COSTUREIRA: <b>'+esc(selected==='TODAS'?'TODAS AS COSTUREIRAS':(lots[0].costureira||selected||'—'))+'</b> · LISTAS: <b>'+esc(uniqueLists.join(' · ')||'—')+'</b>';metas[1].innerHTML='LOTES / DATAS: <b>'+esc(uniqueDates.join(' · ')||'—')+'</b> · PEDIDOS: <b>'+esc(uniqueOrders.join(' · ')||'—')+'</b><br>PEÇAS: <b>'+fmt(totalPieces)+'</b>'}
    ...
    const products=report.querySelector('table.products');if(products){const thead=products.querySelector('thead tr'),tbody=products.querySelector('tbody');if(thead)thead.innerHTML='<th>PRODUTO</th><th>QTD.</th><th>COR</th><th>LOTE / DATA</th><th>NÚMERO DO PEDIDO</th>';if(tbody)tbody.innerHTML=items.length?items.map(x=>`<tr><td>${esc(x.product)}</td><td>${fmt(x.qty)}</td><td>${esc(x.color||'SEM COR')}</td><td>${esc(dateBR(x.__lot.date)||'—')}</td><td class="order-number" data-costura-order="1">${esc(x.__lot.pedido||'—')}</td></tr>`).join(''):'<tr><td colspan="5">Nenhum produto enviado.</td></tr>'}
