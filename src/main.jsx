...
function App(){
 ...
 const [product,setProduct]=useState(''); const [color,setColor]=useState(''); const [qty,setQty]=useState(''); const [search,setSearch]=useState(''); const [view,setView]=useState('list');
 ...
 return <div className={dark?'app dark':'app'}>
  ...
  {calcOpen&&<div className="modal"><div className="sheet"><button className="close" onClick={()=>setCalcOpen(false)}>×</button><small>LANÇAMENTO</small><h2>{product||'Escolher produto'}</h2>{product&&<p className="selected">🎨 {color||'Escolha uma cor'}</p>}
   {!product?<><div className="field"><label>Produto</label><input autoFocus value={product} onChange={e=>setProduct(e.target.value)} placeholder="Digite o produto"/><div className="suggestions">{PRODUCTS.filter(p=>!product||p.toLowerCase().includes(product.toLowerCase())).slice(0,10).map(p=><button key={p} onClick={()=>setProduct(p)}>{p}</button>)}</div></div></>:!color?<div><label>Cor</label><div className="colors">{COLORS.map(c=><button key={c} onClick={()=>setColor(c)}>{c}</button>)}</div></div>:<><div className="current-total">Atual: <b>{items[keyOf(product,color)]?.total||0}</b></div><div className="calc-display">{qty||'0'}</div><div className="calc-grid">{['7','8','9','4','5','6','1','2','3','0','⌫'].map(k=><button key={k} onClick={()=>k==='⌫'?setQty(q=>q.slice(0,-1)):setQty(q=>q+k)}>{k}</button>)}</div><div className="calc-actions"><button className="secondary" onClick={()=>{setColor('');setQty('')}}>OUTRA COR</button><button className="primary" onClick={()=>{addQty(qty);setCalcOpen(false)}}>＋ ADICIONAR</button></div></>}
  </div></div>}
