...
function App(){
const[blackTheme,toggleBlackTheme]=useBlackTheme();
const[tab,setTab]=useState('listas'),[lists,setLists]=useState(()=>read(LISTS_KEY)),[reports,setReports]=useState(()=>read(REPORTS_KEY)),[sendControlOpen,setSendControlOpen]=useState(false),[returnControlOpen,setReturnControlOpen]=useState(false),[returnQty,setReturnQty]=useState({});
...
const totals=useMemo(()=>{const m={};for(const r of reports)m[r.costureira]=(m[r.costureira]||0)+r.items.reduce((s,x)=>s+n(x.qty),0);return m},[reports]);const sewingBalance=useMemo(()=>reports.reduce((s,r)=>s+(r.items||[]).reduce((a,x)=>a+Math.max(0,n(x.qty)-n(x.returnedQty)),0),0),[reports]);
