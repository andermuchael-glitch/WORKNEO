...
const generateReport=()=>{if(!reportList){setMessage('Escolha uma lista no Relatório.');return}if(!costureira){setMessage('Escolha a costureira.');return}const selected=reportList.items.filter(x=>n(x.qty)>0).map(x=>{const q=Math.min(n(sendQty[x.id]),n(x.qty));return q>0?{...x,qty:q,returnedQty:0}:null}).filter(Boolean);if(!selected.length){setMessage('Informe pelo menos uma quantidade para enviar.');return}const a=actor(),grouped=groupProducts(selected),materials=calculateMaterials(selected),r={id:uid(),listId:reportList.id,listName:reportList.name,date:reportDate||today(),costureira,createdAt:new Date().toISOString(),createdBy:a,items:selected,groupedProducts:grouped,materials};saveReports([r,...reports]);const used=new Map(selected.map(x=>[x.id,x.qty]));saveLists(lists.map(l=>l.id!==reportList.id?l:{...l,items:l.items.map(x=>{const q=n(used.get(x.id));return q?{...x,qty:n(x.qty)-q}:x})}));setSendQty({});setMessage('Relatório salvo por '+a.name+': '+fmt(selected.reduce((s,x)=>s+x.qty,0))+' peças e '+fmt(materials.length)+' materiais calculados.');setTab('relatorios')};const returnFromSewing=(reportId,itemId)=>{
 ...
 setMessage('Retorno registrado: '+fmt(requested)+' peça(s) voltaram para a separação e foram baixadas da costura.');
};
