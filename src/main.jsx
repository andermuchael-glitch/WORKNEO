 ...
 const pendingLists=useMemo(()=>{
   return lists.map(l=>{
     ...
     const pendingItems=(l.items||[]).map(item=>{
       const pending=Math.max(0,Number(item.qty||0));
       const key=(r)=>String(r.product||'')+'|'+String(r.pedido||'')+'|'+String(r.color||'');
       const itemKey=String(item.product||'')+'|'+String(item.pedido||'')+'|'+String(item.color||'');
       const sent=rows.filter(r=>r.listId===l.id&&key(r)===itemKey).reduce((sum,r)=>sum+Number(r.qty||0),0);
       const original=pending+sent;
       return {...item,original,sent,pending};
     }).filter(item=>item.pending>0);
     const pending=pendingItems.reduce((sum,item)=>sum+item.pending,0);
     const sentQty=pendingItems.reduce((sum,item)=>sum+item.sent,0);
...
function PublicShare({data,loading,error,filters,setFilters}){
 ...
 <section className="panel"><div className="section-head"><div><h2>📋 LISTAS AINDA NÃO ENVIADAS</h2><p>Agora todos os itens que ainda não foram enviados para a costura aparecem individualmente, com o saldo pendente.</p></div></div>{pendingLists.length?<div style={{display:'grid',gap:14}}>{pendingLists.map(l=><div key={l.id} className="table-wrap"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,padding:'12px 14px',borderBottom:'1px solid #e5e7eb',flexWrap:'wrap'}}><div><strong style={{fontSize:16}}>{l.name}</strong><div style={{fontSize:12,color:'#64748b',marginTop:3}}>{l.pendingItems.length} item(ns) ainda pendente(s)</div></div><div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}><strong style={{color:'#d97706',fontSize:18}}>{fmt(l.pending)} peças</strong><b style={{color:'#d97706'}}>{l.sentQty>0?'ENVIO PARCIAL':'AGUARDANDO ENVIO'}</b></div></div><table><thead><tr><th>PRODUTO</th><th>PEDIDO</th><th>COR</th><th>ORIGINAL</th><th>ENVIADO</th><th>PENDENTE</th></tr></thead><tbody>{l.pendingItems.map((item,i)=><tr key={item.id||item.product+'-'+i}><td><b>{item.product}</b></td><td>{item.pedido||'—'}</td><td>{item.color||'—'}</td><td>{fmt(item.original)}</td><td>{fmt(item.sent)}</td><td><b style={{color:'#d97706'}}>{fmt(item.pending)}</b></td></tr>)}</tbody></table></div>)}</div>:<div className="empty">Nenhuma lista pendente de envio.</div>}</section>
