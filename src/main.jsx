...
import{createProductionShare,listProductionShares,revokeProductionShare,removeProductionShare,loadPublicProductionShare,filterProductionRows}from'./productionShare.js';
import'./styles.css';
...
function PublicShare({data,loading,error,filters,setFilters}){
 ...
 <section className="panel"><div className="section-head"><div><h2>📦 ITENS ENVIADOS À COSTURA</h2><p>Detalhamento por data, produto, quantidade, cor, pedido e costureira.</p></div></div>{rows.length?<div className="table-wrap"><table><thead><tr><th>DATA</th><th>PRODUTO</th><th>QTD.</th><th>COR</th><th>Nº PEDIDO</th><th>COSTUREIRA</th><th>LISTA</th></tr></thead><tbody>{rows.map((r,i)=><tr key={i}><td>{fmtDate(r.date)}</td><td>{r.product}</td><td>{fmt(r.qty)}</td><td>{r.color||'—'}</td><td>{r.pedido||'—'}</td><td>{r.costureira||'—'}</td><td>{r.listName||'—'}</td></tr>)}</tbody></table></div>:<div className="empty">Nenhum item enviado com estes filtros.</div>}</section>
...
const removeShareLink=async id=>{if(!window.confirm('Remover este link desativado permanentemente?'))return;try{const removed=await removeProductionShare(id);if(!removed)throw new Error('Link não encontrado ou sem permissão.');setShareLinks(await listProductionShares());setMessage('Link removido.')}catch(error){console.error(error);setMessage(error?.message||'Não foi possível remover o link.')}};
const logout=async()=>{
