...
const useBlackTheme=()=>{const[blackTheme,setBlackTheme]=useState(()=>localStorage.getItem('workneo-theme-v2')==='black');useEffect(()=>{document.body.classList.toggle('theme-black',blackTheme);document.documentElement.classList.toggle('theme-black',blackTheme);localStorage.setItem('workneo-theme-v2',blackTheme?'black':'light');return()=>{document.body.classList.remove('theme-black');document.documentElement.classList.remove('theme-black')}},[blackTheme]);return[blackTheme,()=>setBlackTheme(v=>!v)]};
 ...
 return <main className="page tracking-page">
   <section className="panel tracking-hero" id="inicio">
     ...
     <div className="tracking-actions">
       ...
       <button className="secondary-action theme-action" onClick={toggleBlackTheme}>{blackTheme?'☀️ TEMA CLARO':'🌙 TEMA BLACK'}</button>
     </div>
...
const[shareType,setShareType]=useState('general'),[shareListId,setShareListId]=useState(''),[shareCostureira,setShareCostureira]=useState(''),[shareStartDate,setShareStartDate]=useState(''),[shareEndDate,setShareEndDate]=useState(''),[shareLinks,setShareLinks]=useState([]),[shareBusy,setShareBusy]=useState(false),[expandedShareLink,setExpandedShareLink]=useState('');
...
const visibleCurrentItems=useMemo(()=>{const items=currentList?.items||[];const hasBalance=x=>n(x.qty)>0||n(x.separacaoQty)>0;if(!projectQuery)return items.filter(hasBalance);return items.filter(x=>hasBalance(x)&&norm([x.product,x.color,x.pedido,x.cliente,x.customer,x.tracking,x.codigoAcompanhamento,x.createdBy?.name,x.createdBy?.email,currentList?.name].join(' ')).includes(projectQuery))},[currentList,projectQuery]);
const filteredReports=useMemo(()=>reports.filter(r=>{if(!projectQuery)return true;const hay=[r.costureira,r.listName,r.date,r.cliente,r.customer,r.tracking,r.codigoAcompanhamento,r.createdBy?.name,r.createdBy?.email,...(r.items||[]).flatMap(x=>[x.product,x.color,x.pedido,x.cliente,x.customer,x.tracking,x.codigoAcompanhamento,x.createdBy?.name,x.createdBy?.email])].join(' ');return norm(hay).includes(projectQuery)}),[reports,projectQuery]);
...
const addItem=()=>{if(!selectedListId||!item.product||n(item.qty)<=0){setMessage('Escolha a lista, o produto e uma quantidade maior que zero.');return}const a=actor();saveLists(lists.map(l=>l.id!==selectedListId?l:{...l,items:[...l.items,{id:uid(),product:item.product,qty:n(item.qty),separacaoQty:n(item.qty),color:item.color||'SEM COR',pedido:String(item.pedido||'').trim(),createdBy:a}]}));setItem({product:'',qty:'',color:'',pedido:''});setMessage('Item lançado por '+a.name+'.')};
...
const generateReport=()=>{if(!reportList){setMessage('Escolha uma lista no Relatório.');return}if(!costureira){setMessage('Escolha a costureira.');return}const selected=reportList.items.filter(x=>Math.max(0,n(x.separacaoQty??x.qty))>0).map(x=>{const q=Math.min(n(sendQty[x.id]),Math.max(0,n(x.separacaoQty??x.qty)));return q>0?{...x,qty:q,returnedQty:0}:null}).filter(Boolean);if(!selected.length){setMessage('Informe pelo menos uma quantidade para enviar.');return}const a=actor(),grouped=groupProducts(selected),materials=calculateMaterials(selected),r={id:uid(),listId:reportList.id,listName:reportList.name,date:reportDate||today(),costureira,createdAt:new Date().toISOString(),createdBy:a,items:selected,groupedProducts:grouped,materials};saveReports([r,...reports]);const used=new Map(selected.map(x=>[x.id,x.qty]));saveLists(lists.map(l=>l.id!==reportList.id?l:{...l,items:l.items.map(x=>{const q=n(used.get(x.id));return q?{...x,qty:Math.max(0,n(x.qty)-q),separacaoQty:Math.max(0,n(x.separacaoQty??x.qty)-q)}:x})}));setSendQty({});setMessage('Relatório salvo por '+a.name+': '+fmt(selected.reduce((s,x)=>s+x.qty,0))+' peças e '+fmt(materials.length)+' materiais calculados.');setTab('relatorios')};const returnFromSewing=(reportId,itemId)=>{
 const report=reports.find(r=>r.id===reportId);
 if(!report){setMessage('Relatório de costura não encontrado.');return}
 const reportItem=(report.items||[]).find(x=>x.id===itemId);
 if(!reportItem){setMessage('Item do relatório não encontrado.');return}
 const alreadyReturned=n(reportItem.returnedQty);
 const inSewing=Math.max(0,n(reportItem.qty)-alreadyReturned);
 const requested=Math.min(n(returnQty[reportId+'|'+itemId]),inSewing);
 if(requested<=0){setMessage('Informe uma quantidade válida para o retorno.');return}
 const a=actor();
 const now=new Date().toISOString();
 const updatedReports=reports.map(r=>{
   if(r.id!==reportId)return r;
   const items=(r.items||[]).map(x=>x.id!==itemId?x:{...x,returnedQty:alreadyReturned+requested});
   const returns=[...(r.returns||[]),{id:uid(),date:now,createdBy:a,archived:false,items:[{itemId,product:reportItem.product,color:reportItem.color||'SEM COR',pedido:reportItem.pedido||'',qty:requested}]}];
   return {...r,items,returns,updatedAt:now};
 });
...
const separationEntries=useMemo(()=>{
 const listMap=new Map(lists.map(l=>[l.id,l]));
 const out=[];
 for(const r of reports){
   for(const ret of (r.returns||[])){
     if(Boolean(ret.archived))continue;
     for(const ri of (ret.items||[])){
       const list=listMap.get(r.listId);
       const listItem=(list?.items||[]).find(x=>x.id===ri.itemId);
       out.push({key:r.id+'|'+ret.id+'|'+(ri.itemId||ri.product),reportId:r.id,returnId:ret.id,itemId:ri.itemId||'',product:ri.product||'—',color:ri.color||'SEM COR',pedido:ri.pedido||'—',qty:n(ri.qty),date:ret.date||r.date,costureira:r.costureira||'—',listName:r.listName||list?.name||'—',separationBalance:n(listItem?.separacaoQty),createdBy:ret.createdBy||r.createdBy||{}});
     }
   }
 }
 return out.sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
},[reports,lists]);
const archivedSeparationEntries=useMemo(()=>{
 const out=[];
 for(const r of reports){
   for(const ret of (r.returns||[])){
     if(!Boolean(ret.archived))continue;
     for(const ri of (ret.items||[])){
       out.push({key:r.id+'|'+ret.id+'|'+(ri.itemId||ri.product),reportId:r.id,returnId:ret.id,itemId:ri.itemId||'',product:ri.product||'—',color:ri.color||'SEM COR',pedido:ri.pedido||'—',qty:n(ri.qty),date:ret.date||r.date,costureira:r.costureira||'—',listName:r.listName||'—',archivedAt:ret.archivedAt,archivedBy:ret.archivedBy||{}});
     }
   }
 }
 return out.sort((a,b)=>String(b.archivedAt||b.date||'').localeCompare(String(a.archivedAt||a.date||'')));
},[reports]);
const archiveSeparationReturn=(reportId,returnId)=>{
 const a=actor(),now=new Date().toISOString();
 saveReports(reports.map(r=>r.id!==reportId?r:{...r,returns:(r.returns||[]).map(ret=>ret.id!==returnId?ret:{...ret,archived:true,archivedAt:now,archivedBy:a}),updatedAt:now}));
 setMessage('Retorno conferido e arquivado na aba SEPARAÇÃO.');
};
const restoreSeparationReturn=(reportId,returnId)=>{
 const now=new Date().toISOString();
...
return <div className="app"><header className="top"><div><div className="brand">WORKNEO</div><div className="subtitle">Lista → Relatório → Materiais → Costura</div></div><div className="top-actions"><div className="status">{isSupabaseConfigured?(session?'☁ '+(session.user.email||'conta conectada'):'☁ login necessário'):'☁ nuvem não configurada'}</div><div className="status">● {syncStatus}</div><button className="backup-btn theme-top-btn" onClick={toggleBlackTheme}>{blackTheme?'☀️ CLARO':'🌙 BLACK'}</button><button className="backup-btn" onClick={()=>downloadBackup(lists,reports,setMessage)}>⬇ BACKUP</button><label className="backup-btn backup-import">⬆ RESTAURAR<input type="file" accept=".json,application/json" onChange={e=>{importBackup(e.target.files?.[0],saveLists,saveReports,setMessage);e.target.value=''}}/></label>{session&&<button className="backup-btn" onClick={logout}>SAIR</button>}</div></header>
...
{!isSupabaseConfigured||session?<nav className="tabs"><button className={tab==='listas'?'active':''} onClick={()=>setTab('listas')}>📋 LISTAS</button><button className={tab==='separacao'?'active':''} onClick={()=>setTab('separacao')}>📦 SEPARAÇÃO</button><button className={tab==='relatorios'?'active':''} onClick={()=>setTab('relatorios')}>📄 RELATÓRIOS</button>{session&&<button className={tab==='equipe'?'active':''} onClick={()=>{setTab('equipe');refreshTeam()}}>👥 EQUIPE</button>}</nav>:null}
...
{(!isSupabaseConfigured||session)&&(tab==='listas'||tab==='relatorios')&&<div className="page project-search-page"><section className="project-search-panel"><label className="project-search"><span>🔎</span><input value={projectSearch} onChange={e=>setProjectSearch(e.target.value)} placeholder="Pesquisar no projeto: pedido, produto, cor, cliente, lista ou relatório" aria-label="Pesquisar no projeto"/>{projectSearch&&<button type="button" onClick={()=>setProjectSearch('')} aria-label="Limpar pesquisa">×</button>}</label><span className="project-search-hint">{tab==='listas'?filteredLists.length+' lista(s) encontrada(s)':filteredReports.length+' relatório(s) encontrado(s)'}</span></section></div>}
{(!isSupabaseConfigured||session)&&tab==='listas'&&<main className="page"><section className="panel"><div className="section-head"><div><h2>LISTA DE SEPARAÇÃO</h2><p>A lista recebe os produtos do pedido. O campo <b>SEPARAÇÃO</b> mostra o saldo disponível para novo envio à costura. O cálculo de materiais acontece somente no Relatório.</p></div></div><div className="create-row"><input value={listName} onChange={e=>setListName(e.target.value)} placeholder="Nome da lista (ex.: 11207 espaço luz)"/><button onClick={createList}>+ NOVA LISTA</button></div><div className="list-grid">{lists.length===0?<div className="empty">Nenhuma lista criada.</div>:filteredLists.length===0?<div className="empty">Nenhuma lista corresponde à pesquisa.</div>:filteredLists.map(l=><button key={l.id} className={'list-card '+(selectedListId===l.id?'selected':'')} onClick={()=>{setSelectedListId(l.id);setReportListId(l.id)}}><b>{l.name}</b><span>{l.items.length} linhas · {fmt(l.items.reduce((s,x)=>s+n(x.qty),0))} peças</span></button>)}</div></section>
{currentList&&<section className="panel"><div className="section-head"><div><h2>{currentList.name}</h2><p>PRODUTO · QUANT. · COR · Nº PEDIDO</p></div><div className="big-number">{fmt(currentList.items.reduce((s,x)=>s+n(x.qty),0))}<small>peças</small></div></div><div className="form-grid"><label>PRODUTO<select value={item.product} onChange={e=>setItem({...item,product:e.target.value})}><option value="">Selecione...</option>{PRODUCTS.map(p=><option key={p}>{p}</option>)}</select></label><label>QUANTIDADE<input type="number" min="1" value={item.qty} onChange={e=>setItem({...item,qty:e.target.value})}/></label><label>COR<select value={item.color} onChange={e=>setItem({...item,color:e.target.value})}><option value="">Selecione...</option>{COLORS.map(c=><option key={c}>{c}</option>)}</select></label><label>Nº PEDIDO<input value={item.pedido} onChange={e=>setItem({...item,pedido:e.target.value})} placeholder="Ex.: 11207"/></label></div><button className="primary" onClick={addItem}>+ ADICIONAR À LISTA</button><div className="table-wrap"><table><thead><tr><th>PRODUTO</th><th>QUANT. LISTA</th><th>SEPARAÇÃO</th><th>COR</th><th>Nº PEDIDO</th><th></th></tr></thead><tbody>{visibleCurrentItems.length?visibleCurrentItems.map(x=><tr key={x.id}><td>{x.product}</td><td>{fmt(x.qty)}</td><td><b>{fmt(x.separacaoQty??x.qty)}</b></td><td>{x.color}</td><td>{x.pedido||'—'}</td><td>{x.createdBy?.name||x.createdBy?.email?<small>{x.createdBy?.name||x.createdBy?.email}</small>:null} {workspace?.role==='admin'&&<button className="danger" onClick={()=>removeItem(x.id)}>Excluir</button>}</td></tr>):<tr><td colSpan="6" className="empty">Esta lista ainda não possui itens.</td></tr>}</tbody></table></div></section>}</main>}
{(!isSupabaseConfigured||session)&&tab==='relatorios'&&<main className="page"><section className="panel"><div className="section-head"><div><h2>GERAR RELATÓRIO A PARTIR DA LISTA</h2><p>Escolha a lista, a costureira e o envio. A ficha técnica calcula automaticamente zíperes, etiquetas, fitas, linhas e demais materiais.</p></div></div><div className="form-grid"><label>LISTA<select value={reportListId} onChange={e=>{setReportListId(e.target.value);setSendQty({})}}><option value="">Selecione a lista...</option>{lists.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label><label>COSTUREIRA<select value={costureira} onChange={e=>setCostureira(e.target.value)}><option value="">Selecione...</option>{SEAMSTRESSES.map(s=><option key={s}>{s}</option>)}</select></label><label>DATA DO ENVIO<input type="date" value={reportDate} onChange={e=>setReportDate(e.target.value)}/></label></div>{reportList?<><div className="hint">Você pode misturar cores e números de pedido. Na hora do relatório, produtos iguais são agrupados; a ficha técnica é aplicada por produto. Uma cor nova continua usando a mesma ficha do produto.</div><div className="table-wrap"><table><thead><tr><th>ENVIAR</th><th>PRODUTO</th><th>DISPONÍVEL</th><th>QTD. ENVIO</th><th>COR</th><th>Nº PEDIDO</th><th>LANÇADO POR</th></tr></thead><tbody>{reportList.items.filter(x=>Math.max(0,n(x.separacaoQty??x.qty))>0).length?reportList.items.filter(x=>Math.max(0,n(x.separacaoQty??x.qty))>0).map(x=><tr key={x.id}><td><input className="check" type="checkbox" checked={n(sendQty[x.id])>0} onChange={e=>setSendQty({...sendQty,[x.id]:e.target.checked?Math.max(0,n(x.separacaoQty??x.qty)):0})}/></td><td>{x.product}</td><td>{x.qty}</td><td><input className="qty" type="number" min="0" max={Math.max(0,n(x.separacaoQty??x.qty))} value={sendQty[x.id]??0} onChange={e=>setSendQty({...sendQty,[x.id]:Math.min(n(e.target.value),Math.max(0,n(x.separacaoQty??x.qty)))})}/></td><td>{x.color}</td><td>{x.pedido||'—'}</td><td>{x.createdBy?.name||x.createdBy?.email||'—'}</td></tr>):<tr><td colSpan="7" className="empty">A lista está sem saldo para envio.</td></tr>}</tbody></table></div><div className="report-total">QUANTIDADE DE PEÇAS: <b>{fmt(totalReport)}</b></div><button className="primary large" onClick={generateReport}>📄 GERAR E SALVAR RELATÓRIO</button></>:<div className="empty">Escolha uma lista para carregar os itens.</div>}</section>
<section className="panel"><div className="section-head"><div><h2>SOMATÓRIA POR COSTUREIRA</h2><p>Clique no quadro da costureira para abrir os detalhes. Os números de enviados, retornados e faltantes também são clicáveis.</p></div></div><div className="summary-grid seamstress-summary-grid">{SEAMSTRESSES.map(s=>{const d=seamstressDetails[s]||{sent:0,returned:0,remaining:0,items:[]};const open=expandedSeamstress===s;const metric=expandedSeamstressMetric;const visible=metric?d.items.filter(x=>metric==='sent'?x.sent>0:metric==='returned'?x.returned>0:x.remaining>0):d.items;return <article className={'summary-card seamstress-card '+(open?'is-open':'')} key={s}><button className="summary-card-head" onClick={()=>setExpandedSeamstress(v=>v===s?'':s)} aria-expanded={open}><span>{s}</span><span className="summary-chevron">{open?'▲':'▼'}</span></button><div className="seamstress-metrics"><button className={'summary-metric sent '+(open&&metric==='sent'?'active':'')} onClick={e=>{e.stopPropagation();setExpandedSeamstress(s);setExpandedSeamstressMetric(v=>v==='sent'?'':'sent')}}><b>{fmt(d.sent)}</b><small>ENVIADAS</small></button><button className={'summary-metric returned '+(open&&metric==='returned'?'active':'')} onClick={e=>{e.stopPropagation();setExpandedSeamstress(s);setExpandedSeamstressMetric(v=>v==='returned'?'':'returned')}}><b>{fmt(d.returned)}</b><small>RETORNARAM</small></button><button className={'summary-metric remaining '+(open&&metric==='remaining'?'active':'')} onClick={e=>{e.stopPropagation();setExpandedSeamstress(s);setExpandedSeamstressMetric(v=>v==='remaining'?'':'remaining')}}><b>{fmt(d.remaining)}</b><small>FALTA RETORNAR</small></button></div>{open&&<div className="seamstress-details"><div className="seamstress-detail-title">{metric==='sent'?'PEÇAS ENVIADAS':metric==='returned'?'PEÇAS QUE JÁ VOLTARAM':'PEÇAS QUE FALTAM RETORNAR'} <span>{fmt(visible.reduce((a,x)=>a+(metric==='sent'?x.sent:metric==='returned'?x.returned:x.remaining),0))} peça(s)</span></div>{visible.length===0?<div className="empty">Nenhum item nesta situação.</div>:<div className="seamstress-items">{visible.map(x=><div className="seamstress-item" key={x.id}><div><b>{x.product}</b><span>{x.color} · Pedido {x.pedido}</span><span>{x.listName} · {fmtDate(x.date)}</span></div><div className="seamstress-item-qty">{metric==='sent'?fmt(x.sent):metric==='returned'?fmt(x.returned):fmt(x.remaining)}<small>{metric==='sent'?'ENVIADAS':metric==='returned'?'RETORNARAM':'A RETORNAR'}</small></div></div>)}</div>}</div>}</article>})}</div></section>
<section className="panel sewing-return-panel"><button className="collapsible-header" onClick={()=>setReturnControlOpen(v=>!v)} aria-expanded={returnControlOpen}><span>↩️ RETORNO DA COSTURA · Dar baixa do que voltou</span><span>{returnControlOpen?'▲':'▼'}</span></button>{!returnControlOpen?<div className="collapsed-summary">Na costura: <b>{fmt(sewingBalance)}</b> peça(s) · Retornaram: <b>{fmt(returnedTotal)}</b> peça(s). Toque para registrar uma devolução.</div>:<div className="collapsible-content"><div className="return-summary"><div><span>NA COSTURA</span><b>{fmt(sewingBalance)}</b></div><div><span>JÁ RETORNARAM</span><b>{fmt(returnedTotal)}</b></div></div>{reports.length===0?<div className="empty">Nenhum envio para registrar retorno.</div>:<div className="saved-list return-list">{reports.map(r=>{const items=(r.items||[]).map(x=>({...x,remaining:Math.max(0,n(x.qty)-n(x.returnedQty))}));const inSewing=items.reduce((s,x)=>s+x.remaining,0);return <article className="saved return-report" key={r.id}><div className="return-report-head"><div><b>{r.costureira}</b><span>{r.listName} · {fmtDate(r.date)} · {fmt(inSewing)} na costura</span><span>{r.createdBy?.name||r.createdBy?.email||'Usuário anterior'} · {fmt((r.items||[]).reduce((s,x)=>s+n(x.qty),0))} enviadas</span></div><span className={'badge '+(inSewing>0?'badge-sewing':'badge-sent')}>{inSewing>0?'NA COSTURA':'RETORNADO'}</span></div>{items.length===0?<div className="empty">Este envio não possui itens.</div>:<div className="table-wrap"><table><thead><tr><th>PRODUTO</th><th>COR</th><th>PEDIDO</th><th>ENVIADO</th><th>RETORNOU</th><th>NA COSTURA</th><th>RETORNAR</th><th></th></tr></thead><tbody>{items.map(x=>{const key=r.id+'|'+x.id;return <tr key={x.id}><td>{x.product}</td><td>{x.color||'SEM COR'}</td><td>{x.pedido||'—'}</td><td>{fmt(x.qty)}</td><td>{fmt(x.returnedQty)}</td><td><b>{fmt(x.remaining)}</b></td><td><input className="qty return-qty" type="number" min="0" max={x.remaining} value={returnQty[key]??''} disabled={x.remaining<=0} onChange={e=>setReturnQty(v=>({...v,[key]:Math.min(n(e.target.value),x.remaining)}))}/></td><td><button className="primary return-btn" disabled={x.remaining<=0||n(returnQty[key])<=0} onClick={()=>returnFromSewing(r.id,x.id)}>↩ DAR BAIXA</button></td></tr>})}</tbody></table></div>}</article>})}</div>}<div className="hint return-note">Cada baixa reduz o saldo <b>NA COSTURA</b> e devolve a mesma quantidade para a lista de separação de origem. O envio original permanece no histórico.</div></div>}</section><section className="panel"><button className="collapsible-header" onClick={()=>setSendControlOpen(v=>!v)} aria-expanded={sendControlOpen}><span>📦 CONTROLE POR ENVIO · Relatórios salvos</span><span>{sendControlOpen?'▲':'▼'}</span></button>{!sendControlOpen?<div className="collapsed-summary">Controle por envio minimizado · {filteredReports.length} envio(s) disponível(is). Toque para consultar.</div>:<div className="collapsible-content"><div className="section-head"><div><p>Cada envio permanece individual e pode gerar os dois relatórios.</p></div></div>{filteredReports.length===0?<div className="empty">{reports.length===0?'Nenhum relatório salvo.':'Nenhum relatório corresponde à pesquisa.'}</div>:<div className="saved-list">{filteredReports.map(r=><article className="saved" key={r.id}><div><b>{r.costureira}</b><span>{r.listName} · {fmtDate(r.date)}</span><span>Lançado por: <b>{r.createdBy?.name||r.createdBy?.email||'Usuário anterior'}</b> · {fmt(r.items.reduce((s,x)=>s+n(x.qty),0))} peças · {r.groupedProducts?.length||groupProducts(r.items).length} produtos agrupados</span></div><div className="actions"><button onClick={()=>printReport(r,'pecas')}>📦 PEÇAS AGRUPADAS</button><button onClick={()=>printReport(r,'materiais')}>🧵 MATERIAIS</button><button className="primary" onClick={()=>printReport(r,'completo')}>📄 COMPLETO</button>{workspace?.role==='admin'&&<button className="danger" onClick={()=>deleteReport(r.id)}>Excluir</button>}</div><details><summary>Ver produtos enviados</summary><div className="table-wrap"><table><thead><tr><th>PRODUTO</th><th>ENVIADO</th><th>RETORNOU</th><th>NA COSTURA</th><th>COR</th><th>Nº PEDIDO</th></tr></thead><tbody>{r.items.map((x,i)=><tr key={i}><td>{x.product}</td><td>{fmt(x.qty)}</td><td>{fmt(x.returnedQty)}</td><td><b>{fmt(Math.max(0,n(x.qty)-n(x.returnedQty)))}</b></td><td>{x.color}</td><td>{x.pedido||'—'}</td></tr>)}</tbody></table></div></details></article>)}</div>}</div>}</section><section className="panel"><div className="section-head"><div><h2>🔗 LINK DE ACOMPANHAMENTO DA PRODUÇÃO</h2><p>Crie um link somente para leitura. O visitante poderá filtrar por período, costureira, produto e lista. Atualização automática.</p></div></div><div className="form-grid"><label>TIPO<select value={shareType} onChange={e=>setShareType(e.target.value)}><option value="general">Produção geral</option><option value="list">Uma lista</option><option value="seamstress">Uma costureira</option></select></label><label>LISTA<select disabled={shareType!=='list'} value={shareListId} onChange={e=>setShareListId(e.target.value)}><option value="">Selecione...</option>{lists.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label><label>COSTUREIRA<select disabled={shareType!=='seamstress'} value={shareCostureira} onChange={e=>setShareCostureira(e.target.value)}><option value="">Selecione...</option>{SEAMSTRESSES.map(s=><option key={s}>{s}</option>)}</select></label><label>DATA INICIAL<input type="date" value={shareStartDate} onChange={e=>setShareStartDate(e.target.value)}/></label><label>DATA FINAL<input type="date" value={shareEndDate} onChange={e=>setShareEndDate(e.target.value)}/></label></div><button className="primary large" disabled={shareBusy} onClick={createShareLink}>🔗 {shareBusy?'CRIANDO...':'CRIAR LINK DE ACOMPANHAMENTO'}</button><div className="share-links-list" style={{marginTop:16}}>{shareLinks.filter(s=>s.active).length===0?<div className="empty">Nenhum link de acompanhamento ativo.</div>:shareLinks.filter(s=>s.active).map(s=>{const open=expandedShareLink===s.id;return <article className={'saved share-link-card '+(open?'expanded':'')} key={s.id}><button type="button" className="share-link-summary" onClick={()=>setExpandedShareLink(open?'':s.id)} aria-expanded={open}><span><b>{s.share_type==='general'?'Produção geral':s.share_type==='list'?'Lista específica':'Costureira específica'}</b><small>{s.start_date||'início'} → {s.end_date||'sem fim'}{s.costureira?' · '+s.costureira:''}</small></span><span className="share-chevron">{open?'▲':'▼'}</span></button>{open&&<div className="share-link-details"><code>{AUTH_REDIRECT_URL+'?acompanhamento='+s.token}</code><div className="actions"><button onClick={()=>navigator.clipboard?.writeText(AUTH_REDIRECT_URL+'?acompanhamento='+s.token)}>📋 COPIAR</button><button className="danger" onClick={()=>revokeShareLink(s.id)}>DESATIVAR</button></div></div>}</article>})}</div></section></main>}
{(!isSupabaseConfigured||session)&&tab==='separacao'&&<main className="page">
<section className="panel separation-panel">
 <div className="section-head">
  <div><h2>📦 SEPARAÇÃO · RETORNOS DA COSTURA</h2><p>Aqui ficam visíveis as peças que <b>voltaram da costura</b>. Confira fisicamente, faça a conferência e depois arquive. Arquivar não apaga o histórico nem reduz o saldo disponível para um novo envio.</p></div>
  <div className="big-number">{fmt(separationEntries.reduce((s,x)=>s+x.qty,0))}<small>peças aguardando conferência</small></div>
 </div>
 ...
 {showArchivedSeparation&&<div className="archived-separation"><div className="section-head"><div><h3>ARQUIVADOS · HISTÓRICO DE CONFERÊNCIA</h3><p>Os registros continuam preservados para consulta.</p></div></div>{archivedSeparationEntries.length===0?<div className="empty">Nenhum retorno arquivado.</div>:<div className="table-wrap separation-table"><table><thead><tr><th>ARQUIVADO EM</th><th>DATA DO RETORNO</th><th>PRODUTO</th><th>COR</th><th>PEDIDO</th><th>QTD.</th><th>COSTUREIRA</th><th></th></tr></thead><tbody>{archivedSeparationEntries.map(x=><tr key={x.key}><td>{fmtDate(String(x.archivedAt||'').slice(0,10))}</td><td>{fmtDate(x.date)}</td><td>{x.product}</td><td>{x.color}</td><td>{x.pedido}</td><td><b>{fmt(x.qty)}</b></td><td>{x.costureira}</td><td><button className="secondary-action small-action" onClick={()=>restoreSeparationReturn(x.reportId,x.returnId)}>↩ REABRIR</button></td></tr>)}</tbody></table></div>}</div>}
</section>
<section className="panel separation-flow"><div className="section-head"><div><h3>FLUXO DA PRODUÇÃO</h3><p>O retorno entra na SEPARAÇÃO e não volta para a LISTA. Depois de conferido, o registro pode ser arquivado.</p></div></div><div className="flow-cards"><div><b>LISTA</b><span>saldo original</span></div><div>→</div><div><b>SEPARAÇÃO</b><span>retornos conferíveis</span></div><div>→</div><div><b>COSTURA</b><span>novo envio</span></div><div>→</div><div><b>RETORNO</b><span>volta para separação</span></div></div></section>
</main>}
...
class AppErrorBoundary extends React.Component{
 ...
 componentDidCatch(error,info){console.error('WORKNEO render:',error,info)}
 render(){
   if(this.state.error)return <main className="page"><section className="panel"><div className="empty"><h2>ERRO AO CARREGAR O WORKNEO</h2><p>{this.state.error?.message||'Erro inesperado.'}</p><button className="primary" onClick={()=>window.location.reload()}>🔄 RECARREGAR</button></div></section></main>;
